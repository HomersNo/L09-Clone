...
public class PostServiceTest extends AbstractTest {
	...
	public void driverPost() {
		...
		for (int i = 0; i < testingData.length; i++)
			this.templatePost((String) testingData[i][0], (String) testingData[i][1], (String) testingData[i][2], (Date) testingData[i][3], (String) testingData[i][4], (String) testingData[i][5], (Double) testingData[i][6], (Double) testingData[i][7], (String) testingData[i][8], (Double) testingData[i][9], (Double) testingData[i][10], (Class<?>) testingData[i][11]);
	}
	...
	// Templates ----------------------------------------------------------
	protected void templatePost(final String username,final String title,final String description,final Date moment,final String type,final String originAddress,final Double originLatitude,final Double originLongitude,final String destinationAddress,final Double destinationLatitude,final Double destinationLongitude, final Class<?> expected) {
		Class<?> caught;
		caught = null;
		try {
			authenticate(username);
			final Post post = postService.create();
			final Place origin = placeService.create();
			final Place destination = placeService.create();
			...
			post.getOrigin().setAddress(originAddress);
			post.getOrigin().setLatitude(originLatitude);
			post.getOrigin().setLongitude(originLongitude);
			post.setDestination(destination);
			post.getDestination().setAddress(destinationAddress);
			post.getOrigin().setLatitude(destinationLatitude);
			post.getOrigin().setLongitude(destinationLongitude);
	...
	protected void templatePostFilter(final String username,final String filter, final Class<?> expected) {
		...
		try {
			authenticate(username);
			Collection<Post> posts = this.postService.findAllFiltered(filter);
		} catch (final Throwable oops) {
			caught = oops.getClass();
		}
		this.checkExceptions(expected, caught);
